...
InterfaceDefStmt* StmtFactory::mk_interface_def(std::string name, Scope scope) {
    InterfaceDefStmt*i = details::emplace_get<InterfaceDefStmt>(mk_fqn(scope, name), m_interfaceDefMap);
...
{
    return details::emplace_get<BaseInitializerStmt>(m_otherStmts, std::move(base), std::move(args));
}
