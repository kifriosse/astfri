...
FunctionDefStmt* StmtFactory::mk_function_def(std::string name, std::vector<ParamVarDefStmt*> params, Type* retType, CompoundStmt* body)
{
...
{
    return details::emplace_get<ClassDefStmt>(stmts_, std::move(name), std::move(vars), std::move(methods), std::move(tparams));
}
...
ForStmt* StmtFactory::mk_for(Stmt* init, Expr* cond, Stmt* step, CompoundStmt* body)
{
...
TranslationUnit* StmtFactory::mk_translation_unit(std::vector<ClassDefStmt*> classes, std::vector<FunctionDefStmt*> functions, std::vector<GlobalVarDefStmt*> globals)
{
    return details::emplace_get<TranslationUnit>(stmts_, std::move(classes), std::move(functions), std::move(globals));
}
