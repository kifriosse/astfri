...
ExprFactory::ExprFactory() : m_typeFactory(&TypeFactory::get_instance())
{
}
...
{
    return details::emplace_get<FunctionCallExpr>(m_otherExpressions, std::move(name), std::move(args));
}

MethodCallExpr* ExprFactory::mk_method_call(Expr* owner, std::string name, std::vector<Expr*> args)
{
    return details::emplace_get<MethodCallExpr>(m_otherExpressions, owner, std::move(name), std::move(args));
}
