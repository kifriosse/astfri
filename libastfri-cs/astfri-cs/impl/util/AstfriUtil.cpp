...
#include <astfri-cs/impl/util/Utils.hpp>
...
Scope mk_scope(const TSNode& node, const SourceFile& currentSrc) {
    ...
    while (! ts_node_is_null(nParent)) {
        ...
        case NodeType::CompilationUnit: {
            ...
            const auto reverseFileNms = std::ranges::views::reverse(currentSrc.fileNms.names);
...
Scope mk_scope(ScopeNode* start, const SourceFile& currentSrc) {
    ...
    return {
        std::vector(std::make_move_iterator(scope.begin()), std::make_move_iterator(scope.end()))
    };
}
...
ParamVarDefStmt* mk_param_def(
    ...
    return StmtFactory::get_instance().mk_param_var_def(extract_text(nName, src), tParam, nullptr);
}
...
ParamSignature discover_params(
    ...
    auto collector = [&](const TSNode& current) -> void {
        ...
        paramsMeta.emplace_back(paramDef, current, ts_node_next_named_sibling(nName));
    };
...
FuncMetadata make_func_metadata(
    ...
    return FuncMetadata{
        ...
        .funcDef = StmtFactory::get_instance()
                       .mk_function_def(std::move(name), std::move(params), retType, nullptr),
        .nFunc = node,
    };
}

std::vector<GenericParam*> make_generic_params(const TSNode& node, const std::string_view src) {
    ...
    auto processGenericParam = [&](const TSNode& current) -> void {
        ...
        const TSNode nName     = child_by_field_name(current, "name");
        GenericParam* genParam = stmtFact.mk_generic_param({}, extract_text(nName, src));
