...
#include <astfri-cs/impl/data/AccessType.hpp>
#include <astfri-cs/impl/data/Identifiers.hpp>
#include <astfri-cs/impl/regs/Maps.hpp>
#include <astfri-cs/impl/util/Common.hpp>
...
Expr* SrcCodeTransformer::visit_identifier(SrcCodeTransformer* self, const TSNode& node) {
    ...
    if (const auto membVar = as<MemberVarDefStmt>(defStmt)) {
...
Expr* SrcCodeTransformer::visit_memb_access(SrcCodeTransformer* self, const TSNode& node) {
    ...
    if (is<BaseExpr>(left))
        qualif = access::Base{};
    else if (is<ThisExpr>(left))
        qualif = access::Instance{};
    else if (is<UnknownExpr>(left))
        return left;
