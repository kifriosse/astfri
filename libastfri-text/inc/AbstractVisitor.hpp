...
concept astfri_node = requires(Node& node) {
...
concept v_astfri_nodes = requires { typename Vector::value_type; }
...
template<typename Vector>
concept v_astfri_members = std::same_as<Vector, std::vector<MemberVarDefStmt*>>
...
template<typename Vector>
concept v_astfri_supertypes = std::same_as<Vector, std::vector<ClassDefStmt*>>
...
public: // TODO tmp
    void process_body(const Stmt* const& stmt);
    void process_condition(const Expr* const& expr);
    ...
    bool try_accept_node(const Node* const& node);
    // -----
...
template<astfri_node Node>
bool AbstractVisitor::try_accept_node(const Node* const& node) {
    if (node) {
