...
void GeneralConfigurator::set_defaults() {
    ...
    outputFilePath_   = std::make_unique<std::stringstream>(fmanager_->default_folder_path());
...
void GeneralConfigurator::process_document(const rj::Value*& doc) {
    const rj::Value* genConf;
...
bool GeneralConfigurator::is_string(
    STRING name,
    const rj::Value*& rjval,
    std::string& getString,
    bool checkStrLen
) {
    const auto& iter = rjval->FindMember(name);
...
bool GeneralConfigurator::is_object(
    STRING name,
    const rj::Value*& rjval,
    const rj::Value*& getObject
) {
    const auto& iter = rjval->FindMember(name);
...
bool GeneralConfigurator::is_array(
    STRING name,
    const rj::Value*& rjval,
    const rj::Value*& getArray
) {
    const auto& iter = rjval->FindMember(name);
...
bool GeneralConfigurator::is_bool(STRING name, const rj::Value*& rjval, bool& getBool) {
    const auto& iter = rjval->FindMember(name);
...
bool GeneralConfigurator::is_int(STRING name, const rj::Value*& rjval, int& getInt) {
    const auto& iter = rjval->FindMember(name);
...
void GeneralConfigurator::process_file_settings(const rj::Value*& settings) {
...
void GeneralConfigurator::process_unknown_phrases(const rj::Value*& phrases) {
    std::string stringVal;
...
void GeneralConfigurator::process_access_modifiers(const rj::Value*& modifiers) {
...
void GeneralConfigurator::process_data_types(const rj::Value*& types) {
    std::string stringVal;
...
void GeneralConfigurator::process_reference_names(const rj::Value*& names) {
    std::string stringVal;
...
void GeneralConfigurator::process_ops_and_seps(const rj::Value*& signs) {
    std::string stringVal;
...
void GeneralConfigurator::process_values(const rj::Value*& values) {
    std::string stringVal;
