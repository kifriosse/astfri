...
void Configurator::set_input_path(std::string const& path)
{
    ...
    if (doc.HasMember("CONFIGURATOR") && doc["CONFIGURATOR"].IsObject())
    {
        ...
        if (conf.HasMember("FILE") && conf["FILE"].IsObject())
        {
            ...
            if (file.HasMember("path") && file["path"].IsString())
            {
                ...
                if (path == "build")
                { // path == "build" -> default path
                    outputFilePath_
                        = std::make_unique<std::stringstream>(defaultOutputFilePath_->str());
                }
                else if (path == "desktop")
                { // path == "desktop" -> active user desktop + text_folder
                    ...
                    outputFilePath_ = std::make_unique<std::stringstream>(
                        "/mnt/c/Users/" + user + "/Desktop/text_output/"
                    );
                }
                else if (path.length() > 0 && path.at(0) == '/')
                { // if path starts with / -> it is absolute path
                    path = path.substr(1);
                    ...
                    { // if path contains any folder
                        ...
                        for (size_t i = 0; i < segments.size(); ++i)
                        {
                            ...
                            { // try to open folder
                                ...
                                { // if partial path exists and it is folder
                                    if (i == segments.size() - 1)
                                    { // it is full path
                                        if (access(newPath.c_str(), W_OK) == 0)
                                        { // if folder has permission to write into itself
                                            std::string out = newPath;
                                            out.append("/");
                                            outputFilePath_
                                                = std::make_unique<std::stringstream>(std::move(out)
                                                );
                                        }
                                        else
                                        {
                                ...
                                { // partial path does not exist
                                    fs::create_directory(newPath);
                                    ++level;
                                    if (i == segments.size() - 1)
                                    { // if it is full created path
                                        if (access(newPath.c_str(), W_OK) == 0)
                                        { // if new folder has permission to write into itself
                                            std::string out = newPath;
                                            out.append("/");
                                            outputFilePath_
                                                = std::make_unique<std::stringstream>(std::move(out)
                                        ...
                                        while (level != 1)
                                        { // need to delete created folder for now
                                            --level;
                            ...
                            { // error in entering folder
                                useDef = true;
                            }
                            if (useDef)
                            { // should use default path?
                                if (level > 0)
                                { // if directories were created, need to delete them
                                    while (level != 1)
                                    {
                                        --level;
                                        newPath = newPath.parent_path();
                    ...
                    { // if path does not contain anything except /
                        outputFilePath_
                            = std::make_unique<std::stringstream>(defaultOutputFilePath_->str());
                    }
                }
                else
                { // if path == "" or is relative -> program/path/text_folder
                    fs::path currentPath = fs::current_path();
                    currentPath.concat("/");
            ...
            { // explicitly set default path, if not present
                outputFilePath_
                    = std::make_unique<std::stringstream>(defaultOutputFilePath_->str());
            }
        ...
        if (conf.HasMember("UNKNOWN_WORD") && conf["UNKNOWN_WORD"].IsString()
            && conf["UNKNOWN_WORD"].GetStringLength() > 0)
        {
        ...
        {
            unknownWordStyle_
                = std::make_unique<std::stringstream>(conf["UNKNOWN_WORD_STYLE"].GetString());
        }
        if (conf.HasMember("INVALID_WORD") && conf["INVALID_WORD"].IsString()
            && conf["INVALID_WORD"].GetStringLength() > 0)
        {
        ...
        {
            invalidWordStyle_
                = std::make_unique<std::stringstream>(conf["INVALID_WORD_STYLE"].GetString());
        }
        if (conf.HasMember("ACCESS_MOD") && conf["ACCESS_MOD"].IsObject())
        {
            ...
            if (acc.HasMember("public") && acc["public"].IsString()
                && acc["public"].GetStringLength() > 0)
            {
                publicWord_ = std::make_unique<std::stringstream>(acc["public"].GetString());
            }
            if (acc.HasMember("private") && acc["private"].IsString()
                && acc["private"].GetStringLength() > 0)
            {
                privateWord_ = std::make_unique<std::stringstream>(acc["private"].GetString());
            }
            if (acc.HasMember("protected") && acc["protected"].IsString()
                && acc["protected"].GetStringLength() > 0)
            {
                protectedWord_ = std::make_unique<std::stringstream>(acc["protected"].GetString());
            }
            if (acc.HasMember("internal") && acc["internal"].IsString()
                && acc["internal"].GetStringLength() > 0)
            {
                internalWord_ = std::make_unique<std::stringstream>(acc["internal"].GetString());
            }
            if (acc.HasMember("attribs") && acc["attribs"].IsString()
                && acc["attribs"].GetStringLength() > 0)
            {
                accAtribWord_ = std::make_unique<std::stringstream>(acc["attribs"].GetString());
            }
            if (acc.HasMember("constructors") && acc["constructors"].IsString()
                && acc["constructors"].GetStringLength() > 0)
            {
                accConstrWord_
                    = std::make_unique<std::stringstream>(acc["constructors"].GetString());
            }
            if (acc.HasMember("destructors") && acc["destructors"].IsString()
                && acc["destructors"].GetStringLength() > 0)
            {
                accDestrWord_ = std::make_unique<std::stringstream>(acc["destructors"].GetString());
            }
            if (acc.HasMember("methods") && acc["methods"].IsString()
                && acc["methods"].GetStringLength() > 0)
            {
        ...
        if (conf.HasMember("DATA_TYPE") && conf["DATA_TYPE"].IsObject())
        {
            ...
            if (dt.HasMember("dynamic") && dt["dynamic"].IsString()
                && dt["dynamic"].GetStringLength() > 0)
            {
            ...
            if (dt.HasMember("float") && dt["float"].IsString()
                && dt["float"].GetStringLength() > 0)
            {
            ...
            if (dt.HasMember("STYLE") && dt["STYLE"].IsObject())
            {
                ...
                {
                    dynamicTypeStyle_
                        = std::make_unique<std::stringstream>(st["dynamic_type_style"].GetString());
                }
                if (st.HasMember("int_type_style") && st["int_type_style"].IsString())
                {
                    intTypeStyle_
                        = std::make_unique<std::stringstream>(st["int_type_style"].GetString());
                }
                if (st.HasMember("float_type_style") && st["float_type_style"].IsString())
                {
                    floatTypeStyle_
                        = std::make_unique<std::stringstream>(st["float_type_style"].GetString());
                }
                if (st.HasMember("char_type_style") && st["char_type_style"].IsString())
                {
                    charTypeStyle_
                        = std::make_unique<std::stringstream>(st["char_type_style"].GetString());
                }
                if (st.HasMember("bool_type_style") && st["bool_type_style"].IsString())
                {
                    boolTypeStyle_
                        = std::make_unique<std::stringstream>(st["bool_type_style"].GetString());
                }
                if (st.HasMember("void_type_style") && st["void_type_style"].IsString())
                {
                    voidTypeStyle_
                        = std::make_unique<std::stringstream>(st["void_type_style"].GetString());
                }
                if (st.HasMember("user_type_style") && st["user_type_style"].IsString())
                {
                    userTypeStyle_
                        = std::make_unique<std::stringstream>(st["user_type_style"].GetString());
                }
            }
        }
        if (conf.HasMember("REF_NAME_STYLE") && conf["REF_NAME_STYLE"].IsObject())
        {
            ...
            {
                defRefNameStyle_
                    = std::make_unique<std::stringstream>(ref["def_ref_name_style"].GetString());
            }
            if (ref.HasMember("gen_param_name_style") && ref["gen_param_name_style"].IsString())
            {
                generParamNameStyle_
                    = std::make_unique<std::stringstream>(ref["gen_param_name_style"].GetString());
            }
            if (ref.HasMember("class_name_style") && ref["class_name_style"].IsString())
            {
                classNameStyle_
                    = std::make_unique<std::stringstream>(ref["class_name_style"].GetString());
            }
            if (ref.HasMember("interface_name_style") && ref["interface_name_style"].IsString())
            {
                interfaceNameStyle_
                    = std::make_unique<std::stringstream>(ref["interface_name_style"].GetString());
            }
            if (ref.HasMember("method_name_style") && ref["method_name_style"].IsString())
            {
                methodNameStyle_
                    = std::make_unique<std::stringstream>(ref["method_name_style"].GetString());
            }
            if (ref.HasMember("function_name_style") && ref["function_name_style"].IsString())
            {
                functionNameStyle_
                    = std::make_unique<std::stringstream>(ref["function_name_style"].GetString());
            }
            if (ref.HasMember("global_var_name_style") && ref["global_var_name_style"].IsString())
            {
                globalVarNameStyle_
                    = std::make_unique<std::stringstream>(ref["global_var_name_style"].GetString());
            }
            if (ref.HasMember("member_var_name_style") && ref["member_var_name_style"].IsString())
            {
                memberVarNameStyle_
                    = std::make_unique<std::stringstream>(ref["member_var_name_style"].GetString());
            }
            if (ref.HasMember("local_var_name_style") && ref["local_var_name_style"].IsString())
            {
                localVarNameStyle_
                    = std::make_unique<std::stringstream>(ref["local_var_name_style"].GetString());
            }
            if (ref.HasMember("param_var_name_style") && ref["param_var_name_style"].IsString())
            {
                paramVarNameStyle_
                    = std::make_unique<std::stringstream>(ref["param_var_name_style"].GetString());
            }
        }
        if (conf.HasMember("OPERATOR") && conf["OPERATOR"].IsObject())
        {
            rj::Value& op = conf["OPERATOR"];
            if (op.HasMember("assign") && op["assign"].IsString()
                && op["assign"].GetStringLength() > 0)
            {
                assignOpWord_ = std::make_unique<std::stringstream>(op["assign"].GetString());
            }
            if (op.HasMember("modulo") && op["modulo"].IsString()
                && op["modulo"].GetStringLength() > 0)
            {
                moduloOpWord_ = std::make_unique<std::stringstream>(op["modulo"].GetString());
            }
            if (op.HasMember("address") && op["address"].IsString()
                && op["address"].GetStringLength() > 0)
            {
                addressOpWord_ = std::make_unique<std::stringstream>(op["address"].GetString());
            }
            if (op.HasMember("deref") && op["deref"].IsString()
                && op["deref"].GetStringLength() > 0)
            {
        ...
        if (conf.HasMember("VALUE") && conf["VALUE"].IsObject())
        {
            ...
            if (val.HasMember("true_val") && val["true_val"].IsString()
                && val["true_val"].GetStringLength() > 0)
            {
                trueVal_ = std::make_unique<std::stringstream>(val["true_val"].GetString());
            }
            if (val.HasMember("false_val") && val["false_val"].IsString()
                && val["false_val"].GetStringLength() > 0)
            {
                falseVal_ = std::make_unique<std::stringstream>(val["false_val"].GetString());
            }
            if (val.HasMember("null_val") && val["null_val"].IsString()
                && val["null_val"].GetStringLength() > 0)
            {
                nullVal_ = std::make_unique<std::stringstream>(val["null_val"].GetString());
            }
            if (val.HasMember("STYLE") && val["STYLE"].IsObject())
            {
                rj::Value const& st = val["STYLE"];
                if (st.HasMember("general_val_style") && st["general_val_style"].IsString())
                {
                    valStyle_
                        = std::make_unique<std::stringstream>(st["general_val_style"].GetString());
                }
                if (st.HasMember("int_val_style") && st["int_val_style"].IsString())
                {
                    intValStyle_
                        = std::make_unique<std::stringstream>(st["int_val_style"].GetString());
                }
                if (st.HasMember("float_val_style") && st["float_val_style"].IsString())
                {
                    floatValStyle_
                        = std::make_unique<std::stringstream>(st["float_val_style"].GetString());
                }
                if (st.HasMember("char_val_style") && st["char_val_style"].IsString())
                {
                    charValStyle_
                        = std::make_unique<std::stringstream>(st["char_val_style"].GetString());
                }
                if (st.HasMember("string_val_style") && st["string_val_style"].IsString())
                {
                    stringValStyle_
                        = std::make_unique<std::stringstream>(st["string_val_style"].GetString());
                }
                if (st.HasMember("bool_val_style") && st["bool_val_style"].IsString())
                {
                    boolValStyle_
                        = std::make_unique<std::stringstream>(st["bool_val_style"].GetString());
                }
                if (st.HasMember("null_val_style") && st["null_val_style"].IsString())
                {
                    nullValStyle_
                        = std::make_unique<std::stringstream>(st["null_val_style"].GetString());
                }
            }
        }
    }
